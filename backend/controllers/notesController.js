...
import send from '../utilities/mailer';
...
export const loginUser = async (req, res) => {
	...
	try {
		const user = await Users.findOne({ emailID: email }, { emailID: true, password: true });
		if (user === null) {
...
export const addNote = async (req, res) => {
	...
	try {
		...
		const result = await Users.updateOne({ _id: userID }, { $push: { notes: note } });
