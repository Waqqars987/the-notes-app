...
export class AuthComponent implements OnInit {
	...
	onSubmit () {
		if (this.isLoginMode && this.authForm.controls.email.invalid && this.authForm.controls.password.invalid) {
			this.dialog.open(MessageComponent, { data: { error: true, message: 'Please enter email & password!' } });
			return;
		}
		...
		authObs.subscribe(
			...
			(errorMessage) => {
				this.isLoading = false;
				this.dialog.open(MessageComponent, { data: { error: true, message: errorMessage } });
			}
		);
		this.authForm.reset();
	}
